    ...
    await this.drizzle.db.insert(users).values([
      {
        id: SINDICO_ID,
        nome: 'Admin Síndico',
        email: 'sindico@condogest.com',
        senha: '$2b$10$ee3qjOOFb2ItCGW1jPWDKu.nARfjIlPCUA2rWupisKq.PJsklbmfa',
        role: 'SINDICO',
      },
      {
        id: JOAO_ID,
        nome: 'João Morador',
        email: 'joao@condogest.com',
        senha: SENHA_HASH,
        role: 'MORADOR',
      },
      {
        id: MARIA_ID,
        nome: 'Maria Moradora',
        email: 'maria@condogest.com',
        senha: SENHA_HASH,
        role: 'MORADOR',
      },
    ]).onConflictDoNothing();

    this.logger.log('  ✓ Usuários inseridos (3)');
  }
