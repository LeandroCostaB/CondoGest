...
export class CoreEventConsumerService implements OnApplicationBootstrap {
  ...
  private async handleEvent(event: CoreEventName, payload: Record<string, unknown>): Promise<void> {
    switch (event) {
      case CoreEventName.MORADOR_CRIADO:
      case CoreEventName.MORADOR_ATUALIZADO:
        await this.upsertResident(payload);
        break;
      case CoreEventName.MORADOR_DELETADO:
        await this.deleteResident(payload.id as string);
        break;
      case CoreEventName.APARTAMENTO_CRIADO:
      case CoreEventName.APARTAMENTO_ATUALIZADO:
        await this.upsertApartment(payload);
        break;
      case CoreEventName.APARTAMENTO_DELETADO:
        await this.deleteApartment(payload.id as string);
        break;
      case CoreEventName.CONDOMINIO_CRIADO:
      case CoreEventName.CONDOMINIO_ATUALIZADO:
        await this.upsertCondominium(payload);
        break;
      case CoreEventName.CONDOMINIO_DELETADO:
