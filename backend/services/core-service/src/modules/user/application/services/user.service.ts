...
export class UserService {
  ...
  async update(id: string, data: { nome?: string; email?: string; senha?: string }): Promise<UserDto> {
    ...
    await this.messagingService.publishCoreEvent(CoreEventName.MORADOR_ATUALIZADO, {
      id: updated.id,
      nome: updated.nome,
      email: updated.email,
      role: updated.role,
  ...
  async delete(id: string): Promise<{ message: string }> {
    ...
    await this.messagingService.publishCoreEvent(CoreEventName.MORADOR_DELETADO, { id });
