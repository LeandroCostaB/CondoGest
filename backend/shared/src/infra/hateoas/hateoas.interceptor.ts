...
export class HateoasInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    const listOptions = this.reflector.get<HateoasListOptions>(HATEOAS_LIST_KEY, context.getHandler());
    const itemOptions = this.reflector.get<HateoasItemOptions>(HATEOAS_ITEM_KEY, context.getHandler());
  ...
  private transformItem(item: Record<string, unknown> | null, options: HateoasItemOptions) {
    if (!item) return null;
    return {
      ...item,
      _links: options.itemLinks(item),
