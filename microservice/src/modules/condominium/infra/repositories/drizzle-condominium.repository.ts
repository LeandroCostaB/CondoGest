...
import type { CondominiumRepository } from "@condominium/domain/repositories/condominium-repository.interface";
...
export class DrizzleCondominiumRepository implements CondominiumRepository {
  ...
  async findById(id: string): Promise<Condominium | null> {
    ...
    return Condominium.restore({
      ...result[0],
      status: result[0].status as CondominiumStatus,
    });
  }
